...
namespace SmartLin.LearningCSharp.ErrorAndException
{
    class Program
    {
        static void Main()
        {
            ...
            {
                newUser = User.Register("3180707001", "MyPassword7001", "3180707001@fjtcm.edu.cn");
                newUser.Activate();
            }                                                                                           //若try语句块出现异常，则将进入catch语句块，捕捉并处理异常；
            catch (ApplicationException ex)                                                             //每个catch语句块将捕捉类型最匹配的异常；
            {                                                                                           //派生层次最低、针对性最强的异常类型，宜最先捕捉；
                Console.WriteLine(ex.Message);
                Console.WriteLine(ex.InnerException.Message);                                           //访问异常所包含的内部异常；
                Console.Read();
                return;
            }
            catch (Exception ex)                                                                        //派生层次最高、通用性最强的异常类型，宜最后捕捉；
            {
                Console.WriteLine("发生其它错误。错误消息已存入日志。请联系技术人员。", ex.Message);
                Console.Read();
                return;
            }
            Console.WriteLine                                                                           //后续语句；
                ("用户{0}完成注册并激活。"
                , newUser.Number);
            Console.Read();
        }

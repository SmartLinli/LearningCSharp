	...
	class Program
	{
		static void Main()
		{
			...
			Student[] newStudents =
			{
                ...
                //new Student("3210707008", "刘浩雄"),                                  //此后班级对象的学生属性数组的长度将为10，但非空元素个数将为9；
                new Student("3210707009", "程琨耀"),
                new Student("3210707010", "李健铭")
            };
            im21.AddStudents(newStudents);
			/*排序；*/
			Array.Sort(im21.Students);                                                  //调用Array类的Sort静态方法，对数组进行冒泡排序，该数组还需实现IComparable接口；
            Array.Sort                                                                  //调用Sort方法的另一重载，实现更多排序选项；该方法还需一个实现了IComparer<>的类作为参数，用于实现更多排序选项；
                (im21.Students, new StudentComparer(StudentComparer.CompareType.NAME)); 
            /*显示排序结果；*/

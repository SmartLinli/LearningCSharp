...
namespace SmartLin.LearningCSharp.Installer
{
    ...
    public class ValidationService
    {
        ...
        private static object GetControlContent(Control control)
        {
            ...
            if (control is NumericUpDown)                                                           //若控件为数值增减框；
            {
                controlContent = (control as NumericUpDown).Value;                                  //则获取数值增减框的值；
            }
        ...
        /// <param name="form">窗体</param>
        /// <param name="control">控件</param>
        public static void SetMaxMinValue<C>(Form form, C control) where C : NumericUpDown
        {
            var decimalValidationAttribute =                                                        //获取数值验证特性；
                GetValidationAttribute(form, control)
                as DecimalValidationAttribute;
            control.Maximum = decimalValidationAttribute.MaxValue;                                  //设置控件的最大值；
            control.Minimum = decimalValidationAttribute.MinValue;                                  //设置控件的最小值；
        }
    }
}
...
namespace SmartLin.LearningCSharp.eXtensibleMarkupLanguage
{
    ...
    public partial class frm_CourseManagement
    {
        ...
        {
            this.lsb_CourseNames.Items.Clear();
            this._CourseRepository = CourseRepositoryFactory.CreateRepository();
            foreach (Course course in this._CourseRepository.FindAll())
            {
                this.lsb_CourseNames.Items.Add(course);                                         //将对象加入列表框的项目集合；对象若重写ToString方法，则可将ToString方法结果显示为列表项；
            }
        }
        /// <summary>
        /// 重置控件；
        ...
        /// 载入课程；
        /// </summary>
        private void LoadCourse(object selectedItem)
        {
            ...
            string number = (selectedItem as Course).Number;
            this._Course = this._CourseRepository.FindBy(number);
            this.pnl_Course.Enabled = true;
        ...
        private void SetCourseCreditDefault(decimal defaultValue)
        {
            CourseService.SetDefaultCredit(defaultValue);
        }
        /// <summary>
        /// 获取课程学分默认值；
        /// </summary>
        /// <returns>默认值</returns>
        private decimal GetCourseCreditDefault()
        {
            return CourseService.GetDefaultCredit();
        }
        /// <summary>
        /// 包含应选中项目的批量添加；
        /// </summary>
        ...
        private void OpenTxtFile(TextBox textBox)
        {
            ...
            openFileDialog.FileName = string.Format
                ("{0}《{1}》课程描述"
                , this._Course.Number
                , this._Course.Name);
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                textBox.Text = File.ReadAllText(fileName);                                      //调用File类的ReadAllText静态方法，按照指定文件名读取文本文件；                                               
            }
        }
        /// <summary>
        /// 保存文本文件；
        /// </summary>
        /// <param name="textBox"></param>
        private void SaveTxtFile(TextBox textBox)
        {
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                File.WriteAllText(fileName, textBox.Text);                                      //调用File类的WriteAllText静态方法，按照指定文件名写入文本文件；                                               
            }
        }
        /// <summary>
        /// 重绘工具栏
        ...
        {
            Color currentColor = richTextBox.SelectionColor;                                    //获取富文本框选中文字的颜色；
            ...
            {
                currentColor = colorDialog.Color;                                               //获取颜色对话框选定的颜色；
            }
        ...
        private void SetFont(RichTextBox richTextBox)
        {
            ...
            {
                currentFont = fontDialog.Font;                                                  //获取字体对话框选定的字体；
            }
        ...
        {
            string oldNumber = this._Course.Number;
            ...
            this.txb_CourseDescription.Text = this._Course.Description;
            this._CourseRepository.Update(oldNumber, this._Course);
            this.LoadCourses();
        }
    }
}
